$(function(){
    ...
    canvas.width = settings.displaySize;
    canvas.height = settings.displaySize;
    ...
    // Position de l'agent
    var position = {
        x: Math.random()*settings.displaySize,
        y: Math.random()*settings.displaySize,
        update: function(){
            this.x = Math.random()*settings.displaySize;
            this.y = Math.random()*settings.displaySize;
        }
    }
    ...
    function createAgent() {
        ...
        agent.update = function(distanceToMouseSquared) {
            ...
            if(agent.x >= settings.displaySize || agent.x <= 0){

                agent.xIncrement = - agent.xIncrement;
                agent.saveXIncr = agent.xIncrement;

            } else if( agent.y >= settings.displaySize || agent.y <= 0 ) {
    ...
    function step() {
        ...
        if(!settings.stopAnimation){
            ...
            ctx.fillRect(0, 0, settings.displaySize, settings.displaySize);
